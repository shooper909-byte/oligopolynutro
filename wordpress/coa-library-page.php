...
/**
 * Show specimen cards while the library holds no real records.
 *
 * These are NOT placeholder records. Every value in them is a symbol, not a
 * value: the batch reads `OP-######-XXX`, the product reads "Compound name",
 * dates read "DD Month YYYY". There is no date, no laboratory, no result and no
 * document link to mistake for real data, and they carry a SPECIMEN band.
 *
 * They exist so the page demonstrates its own layout and, more usefully, so a
 * customer can see what each document status means before they search. One card
 * per status doubles as the status legend.
 *
 * They are display-only: they are not in the record set, so a search can never
 * match one, and they disappear on their own the moment a real record is
 * published. Return false to switch them off.
 */
function opl_cl_specimens() {
	return true;
}
...
function opl_cl_hero( $batch ) {
	...
	// A transcription aid, not a record. It shows the SHAPE of the ID printed
	// on the card so a customer can check they have copied every character,
	// which is the commonest reason a scan-and-search fails. Deliberately says
	// nothing about what the segments mean - that is not established here.
	$out .= '<p class="oplcl-mask">'
		. '<span class="oplcl-mask-label">Batch IDs look like</span>'
		. '<code class="oplcl-mask-code">'
		. '<b>OP</b><i>&ndash;</i><b>######</b><i>&ndash;</i><b>XXX</b>'
		. '</code>'
		. '<span class="oplcl-mask-note">two letters, six digits, then a short compound code, '
		. 'joined by hyphens</span></p>';
...
/**
 * Specimen cards shown only while the library is empty.
 *
 * Every field is a symbol. There is no batch number, no date, no laboratory, no
 * result and no link. Each card demonstrates one document status, so the block
 * reads as the status legend as well as a layout preview.
 *
 * These are not records and are not searchable - they live outside
 * opl_cl_records() entirely, so opl_cl_lookup() can never return one.
 */
function opl_cl_specimen_block() {
	if ( ! opl_cl_specimens() ) {
		return '';
	}

	$specimens = array(
		array( 'Documents Available', 'A certificate is published for this exact batch.' ),
		array( 'Partial Documentation', 'Some documents are published; others are still in review.' ),
		array( 'Pending', 'The batch is logged, but no document is published yet.' ),
		array( 'Superseded', 'A corrected document has replaced this one. Both stay available.' ),
		array( 'Archived', 'An older batch, kept on file rather than removed.' ),
	);

	$out = '<div class="oplcl-spec-wrap">';
	$out .= '<h3 class="oplcl-spec-title">What a record will look like</h3>';
	$out .= '<p class="oplcl-spec-intro">These are layout specimens, not records. Every value below '
		. 'is a symbol standing in for a field &mdash; there is no batch, date, laboratory or '
		. 'document behind them. They show how each document status is displayed.</p>';
	$out .= '<ul class="oplcl-spec-grid">';

	foreach ( $specimens as $specimen ) {
		list( $status, $meaning ) = $specimen;
		$tone = opl_cl_status_tone( $status );

		$out .= '<li class="oplcl-spec" aria-label="Specimen card, status ' . esc_attr( $status ) . '">';
		$out .= '<span class="oplcl-spec-band">Specimen &mdash; not a record</span>';
		$out .= '<p class="oplcl-spec-name">Compound name <span class="oplcl-spec-sym">## mg</span></p>';
		$out .= '<p class="oplcl-spec-line">Batch ID: <code>OP-######-XXX</code></p>';
		$out .= '<p class="oplcl-spec-line">Report date: <code>DD Month YYYY</code></p>';
		$out .= opl_cl_badge( $status, $tone );
		$out .= '<p class="oplcl-spec-meaning">' . esc_html( $meaning ) . '</p>';
		$out .= '</li>';
	}
 ...
 *
 * The brief allows breadcrumb markup, and an earlier revision of this file
 * emitted a BreadcrumbList. Checked against the deployed page, that was a
 * mistake: the site already emits two breadcrumb graphs - one from Rank Math
 * and one from the `opb-phase3-breadcrumb-schema` snippet - and the latter uses
 * the SAME `@id` (`<url>#breadcrumb`). Two nodes sharing an `@id` while
 * disagreeing on the trail name ("Certificate Library" vs "Certificates &
 * Documentation") is a genuine conflict, not a harmless duplicate.
 *
 * Adding a third graph would make that worse, so this page contributes none and
 * leaves the site's existing markup alone. Kept as a function so the decision is
...
/**
 * Drop the site-wide FAQ schema on this page only.
 *
 * `opb-phase3-faq-schema` hard-codes three questions ("What is a COA?", "Do
 * COAs change product intent?", "Where can I find product documentation?") that
 * lived on the old version of this page. They are not on the redesigned page,
 * so the markup now describes content no visitor can see - which breaks
 * Google's rule that FAQ markup must correspond to visible content, and
 * contradicts the brief's "add structured data only when accurate".
 *
 * The snippet that emits it is site-wide and not in this repository, so rather
 * than change its behaviour everywhere, the block is removed from the finished
 * HTML of page 1652 alone. Every other page keeps it exactly as before.
 *
 * If those FAQs are wanted back here, the answer is to put the questions on the
 * page and let the schema describe them again - not to re-add the markup.
 */
add_action( 'template_redirect', 'opl_cl_buffer_start', PHP_INT_MAX );

function opl_cl_buffer_start() {
	if ( is_admin() || is_feed() || is_robots() || ! opl_cl_is_page() ) {
		return;
	}

	ob_start( 'opl_cl_strip_stale_faq' );
}

function opl_cl_strip_stale_faq( $html ) {
	if ( ! is_string( $html ) || false === strpos( $html, 'opb-phase3-faq-schema' ) ) {
		return $html;
	}

	$out = preg_replace(
		'#<script[^>]*id="opb-phase3-faq-schema"[^>]*>.*?</script>#is',
		'<!-- opl-cl: page-scoped removal of FAQ schema whose questions are not on this page -->',
		$html,
		1
	);
...
function opl_cl_css() {
	...
	// Batch ID shape guide.
	. '.oplcl-mask{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;'
	. 'gap:6px 12px;max-width:760px;margin:14px auto 0}'
	. '.oplcl-mask-label{color:var(--mut);font-size:15px}'
	. '.oplcl-mask-code{display:inline-flex;align-items:center;gap:3px;padding:6px 12px;'
	. 'border:1px dashed var(--line);border-radius:9px;background:var(--s1);'
	. 'font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:16px;'
	. 'letter-spacing:.06em}'
	. '.oplcl-mask-code b{color:var(--acc);font-weight:700}'
	. '.oplcl-mask-code i{color:var(--mut);font-style:normal}'
	. '.oplcl-mask-note{flex:1 1 100%;color:var(--mut);font-size:15px}'
	...
	// Specimen cards. Visibly not records: dashed edges, muted, a standing band.
	. '.oplcl-spec-wrap{margin-top:30px;padding-top:26px;border-top:1px solid var(--line2)}'
	. '.oplcl-spec-title{font-size:20px;font-weight:900}'
	. '.oplcl-spec-intro{max-width:760px;margin:0 0 18px;color:var(--mut)}'
	. '.oplcl-spec-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;'
	. 'margin:0;padding:0;list-style:none}'
	. '.oplcl-spec{position:relative;padding:18px;border:1px dashed var(--line2);'
	. 'border-radius:14px;background:repeating-linear-gradient(135deg,var(--s1),var(--s1) 12px,'
	. 'rgba(190,198,214,.035) 12px,rgba(190,198,214,.035) 24px)}'
	. '.oplcl-spec-band{display:inline-block;margin-bottom:11px;padding:4px 10px;border-radius:999px;'
	. 'border:1px solid var(--line2);background:var(--bg);color:var(--mut);font-size:12px;'
	. 'font-weight:800;letter-spacing:.09em;text-transform:uppercase}'
	. '.oplcl-spec-name{margin:0 0 6px;color:var(--mut);font-size:17px;font-weight:800}'
	. '.oplcl-spec-sym{color:#7C8598;font-weight:700}'
	. '.oplcl-spec-line{margin:0 0 5px;color:var(--mut);font-size:15px}'
	. '.oplcl-spec-line code{padding:1px 6px;border-radius:5px;background:rgba(190,198,214,.08);'
	. 'color:#8F98AB;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:14px}'
	. '.oplcl-spec .oplcl-badge{margin-top:8px;opacity:.85}'
	. '.oplcl-spec-meaning{margin:10px 0 0;color:var(--mut);font-size:15px;line-height:1.5}'
	...
	. '.oplcl-grid,.oplcl-steps,.oplcl-spec-grid{grid-template-columns:repeat(2,minmax(0,1fr))}'
	...
	. '.oplcl-grid,.oplcl-steps,.oplcl-spec-grid{grid-template-columns:1fr}'
...
function opl_cl_render() {
	...
	// Everything lives inside the wrapper. Page 1652 carries the shortcode as a
	// Gutenberg shortcode block, so `do_blocks()` unhooks `wpautop` and the
	// output is left alone - but keeping the <style> inside the div means the
	// markup survives even if it is ever pasted as classic content, where
	// wpautop would otherwise wrap the loose tags in stray paragraphs.
 ...
 */
/** True when this request is a batch lookup rather than the page itself. */
function opl_cl_is_lookup() {
	return ( opl_cl_is_page() && isset( $_GET['batch'] ) );
}

/**
 * Ask Rank Math for noindex on a lookup, rather than printing a second tag.
 *
 * Rank Math already emits `robots` and `canonical` on this page. Echoing our
 * own produced two of each - `noindex,follow` next to `follow, index` - and
 * while a crawler resolves that conflict in favour of the restrictive one, it
 * is not something to rely on. Going through the plugin's own filters means
 * exactly one of each tag is emitted, saying the right thing.
 */
add_filter( 'rank_math/frontend/robots', 'opl_cl_rm_robots', 99 );

function opl_cl_rm_robots( $robots ) {
	if ( ! opl_cl_is_lookup() ) {
		return $robots;
	}
...
/**
 * Fallback for a site without Rank Math.
 *
 * Only prints when nothing else has claimed the robots tag, so it cannot
 * reintroduce the duplicate it was written to remove.
 */
add_action( 'wp_head', 'opl_cl_robots', 1 );

function opl_cl_robots() {
	if ( ! opl_cl_is_lookup() || class_exists( 'RankMath' ) ) {
		return;
	}
