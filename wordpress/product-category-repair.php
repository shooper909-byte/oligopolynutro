<?php
/**
 * OligoPoly - one-time product category repair
 *
 * THE PROBLEM
 * -----------
 * Category navigation leads nowhere. `/product-category/metabolic-research/`
 * and every other research category return WooCommerce's "no products found".
 *
 * It is not a broken query and not a visibility bug. Audited 2026-08-23:
 *
 *   - 14 of the 15 sellable containers (kits, stacks, build-your-own bundles)
 *     have NO product category at all. Only 3454 has one. They therefore appear
 *     solely under /product-category/uncategorized/, which lists all 15.
 *
 *   - The 10 individual compounds DO carry categories - 7 of them are in
 *     `metabolic-research` - but they are flagged "not sold separately", which
 *     excludes them from catalogue listings by design. That is correct: they
 *     cannot be bought alone, so listing them in a shop category would be a
 *     dead end.
 *
 * The result is a catalogue whose categories contain the products that must not
 * be listed, and exclude the ones that can actually be bought.
 *
 * THE FIX
 * -------
 * Give each container the categories its own contents justify. Nothing is
 * invented: a kit inherits the research categories of the compound inside it,
 * read live from WooCommerce.
 *
 * Named stacks are the one exception. `Neurocognitive Pathways Stack` and
 * `Regenerative Biology Stack` contain the SAME three compounds (NAD+, GHK-Cu,
 * Selank), so contents cannot tell them apart - inheriting the union would file
 * the neurocognitive stack under Metabolic Research. Their own names state the
 * research area, so that is used instead, via an explicit map below.
 *
 * Build-your-own bundles span every area by design, so they get the umbrella
 * categories only. Filing them under all four research areas would put the same
 * three bundles at the top of every category page.
 *
 * SAFETY
 * ------
 *   - Terms are APPENDED, with exactly one exception: "Uncategorized" is
 *     removed from any container that has real categories, because all 15
 *     carried it and would otherwise be filed in two places at once. Never
 *     removed if it would leave the product with no category. Every removal is
 *     recorded in the log.
 *   - Products themselves are untouched: no name, price, stock, description,
 *     SKU or meta is written.
 *   - It runs ONCE, guarded by an option, and records what it did so the change
 *     is auditable and reversible.
 *   - Nothing runs on a front-end request. It fires on `admin_init` only.
 *
 * RE-RUNNING
 * ----------
 * The guard is `opl_pcat_done_v3`. Earlier revisions used `opl_pcat_done` and
 * `opl_pcat_done_v2`; each left work undone, so the key is bumped rather than
 * reused. Delete `opl_pcat_done_v3` to run it again after editing the maps.
 *
 * Everything here is idempotent: a product that already has its categories is
 * skipped, and a term that no longer exists cannot be deleted twice.
 *
 * TO REVERSE
 * ----------
 * The option `opl_pcat_log` holds every product ID and the exact term IDs added
 * to it. Remove those terms in WooCommerce > Products, or delete the option and
 * re-run after editing the map.
 *
 * AFTER IT RUNS
 * -------------
 * Deactivate and delete this snippet. It is a migration, not a feature.
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/** Categories every sellable container should carry. */
function opl_pcat_umbrella() {
	return array( 'research-compounds', 'research-products' );
}

/**
 * Multi-vial products also belong on the "Stacks" shelf.
 *
 * `/product-category/research-stacks/` is linked as "Research Stacks" from a
 * navigation row on 11 pages and is currently empty - a sitewide dead end.
 * Curated stacks and build-your-own bundles are both multi-vial configurable
 * products, which is what a customer clicking "Stacks" is looking for, so both
 * go here. Single-compound kits do not: a six-pack of one compound is a kit,
 * not a stack.
 */
function opl_pcat_stacks_slug() {
	return 'research-stacks';
}

/**
 * Terms that must never be deleted by the sweep below, whatever their count.
 *
 *   - the six this migration fills
 *   - `research-stacks`, filled here too
 *   - the five retired supplement categories: they return HTTP 410 Gone on
 *     purpose, which is the correct way to retire a URL. Deleting the term
 *     would turn a deliberate 410 into an accidental 404 and lose that signal
 *   - four categories still linked from product cards on /research-catalog/.
 *     They are empty and no product fits them, but deleting a term something
 *     links to trades an empty page for a 404, which is worse. They need the
 *     card markup changed first - a content decision, not a cleanup
 *   - `uncategorized`, which WordPress requires
 */
function opl_pcat_protected() {
	return array(
		// Filled by this migration.
		'metabolic-research',
		'cellular-research',
		'longevity-research',
		'cognitive-research',
		'research-compounds',
		'research-products',
		'research-stacks',

		// Retired on purpose, serving 410.
		'vitamins',
		'longevity',
		'performance',
		'cognitive-support',
		'wellness',

		// Still linked from catalogue cards.
		'growth-hormone-research',
		'immune-research',
		'recovery-research',
		'research-blends-cat',

		// WordPress default.
		'uncategorized',
	);
}

/**
 * Research area by stack name, where contents cannot distinguish them.
 *
 * Keyed by a lowercase substring of the product name.
 */
function opl_pcat_stack_map() {
	return array(
		'metabolic pathways'      => array( 'metabolic-research' ),
		'cellular energy'         => array( 'cellular-research' ),
		'neurocognitive pathways' => array( 'cognitive-research' ),
		'regenerative biology'    => array( 'cellular-research', 'longevity-research' ),
	);
}

/** Research categories that describe an area of study, not a catalogue shelf. */
function opl_pcat_research_slugs() {
	return array( 'metabolic-research', 'cellular-research', 'longevity-research', 'cognitive-research' );
}

/** Work out the right categories for one container. */
function opl_pcat_plan_for( $product ) {
	$name = strtolower( $product->get_name() );

	// Build-your-own bundles span every research area by design, so they get no
	// area of their own - filing them under all four would put the same three
	// bundles at the top of every category page. They are stacks, though.
	if ( false !== strpos( $name, 'build your research bundle' ) ) {
		return array_merge( opl_pcat_umbrella(), array( opl_pcat_stacks_slug() ) );
	}

	// A named stack states its own area.
	foreach ( opl_pcat_stack_map() as $needle => $slugs ) {
		if ( false !== strpos( $name, $needle ) ) {
			return array_merge( $slugs, opl_pcat_umbrella(), array( opl_pcat_stacks_slug() ) );
		}
	}

	// Everything else inherits from what is actually inside it.
	$research = opl_pcat_research_slugs();
	$found    = array();

	if ( is_callable( array( $product, 'get_child_items' ) ) ) {
		foreach ( $product->get_child_items() as $item ) {
			$child = is_callable( array( $item, 'get_product' ) ) ? $item->get_product() : null;

			if ( ! $child ) {
				continue;
			}

			$terms = wp_get_object_terms( $child->get_id(), 'product_cat', array( 'fields' => 'slugs' ) );

			if ( is_wp_error( $terms ) ) {
				continue;
			}

			foreach ( $terms as $slug ) {
				if ( in_array( $slug, $research, true ) ) {
					$found[ $slug ] = true;
				}
			}
		}
	}

	return array_merge( array_keys( $found ), opl_pcat_umbrella() );
}

add_action( 'admin_init', 'opl_pcat_run' );

function opl_pcat_run() {
	// Version the guard. An earlier revision of this file ran with the key
	// `opl_pcat_done`; it assigned the research categories but predates the
	// Stacks shelf, the Uncategorized removal and the deletion sweep. Reusing
	// that key would make this revision a silent no-op on any site where the
	// first one already ran.
	if ( get_option( 'opl_pcat_done_v3' ) ) {
		return;
	}

	if ( ! function_exists( 'wc_get_product' ) || ! current_user_can( 'manage_woocommerce' ) ) {
		return;
	}

	$containers = get_posts(
		array(
			'post_type'        => 'product',
			'post_status'      => 'publish',
			'numberposts'      => 200,
			'fields'           => 'ids',
			'suppress_filters' => false,
			'tax_query'        => array(
				array(
					'taxonomy' => 'product_type',
					'field'    => 'slug',
					'terms'    => 'mix-and-match',
				),
			),
		)
	);

	$log   = array();
	$touch = array();

	foreach ( (array) $containers as $id ) {
		$product = wc_get_product( $id );

		if ( ! $product ) {
			continue;
		}

		$want = opl_pcat_plan_for( $product );

		if ( empty( $want ) ) {
			continue;
		}

		$have = wp_get_object_terms( $id, 'product_cat', array( 'fields' => 'slugs' ) );
		$have = is_wp_error( $have ) ? array() : $have;

		$missing  = array_values( array_diff( $want, $have ) );
		$term_ids = array();

		// Resolve slugs to IDs; skip any slug that does not exist rather than
		// creating a category nobody asked for.
		foreach ( $missing as $slug ) {
			$term = get_term_by( 'slug', $slug, 'product_cat' );

			if ( $term && ! is_wp_error( $term ) ) {
				$term_ids[] = (int) $term->term_id;
			}
		}

		if ( ! empty( $term_ids ) ) {
			// APPEND. The final `true` is what makes this non-destructive.
			wp_set_object_terms( $id, $term_ids, 'product_cat', true );
		}

		// Drop "Uncategorized" from any container that now has real categories.
		//
		// This deliberately runs for EVERY container, not only one that gained a
		// term in this pass. An earlier revision nested it inside the "gained
		// something" branch, so the eight kits - which already had their
		// categories from a previous run - were skipped and kept Uncategorized
		// alongside the correct category.
		//
		// It is still narrow: only the default term, only when a real category
		// remains, and recorded in the log so it can be put back.
		$dropped = array();
		$default = get_term_by( 'slug', 'uncategorized', 'product_cat' );

		if ( $default && ! is_wp_error( $default ) ) {
			$current = wp_get_object_terms( $id, 'product_cat', array( 'fields' => 'ids' ) );

			if ( ! is_wp_error( $current ) && in_array( (int) $default->term_id, array_map( 'intval', $current ), true ) ) {
				$keep = array_values( array_diff( array_map( 'intval', $current ), array( (int) $default->term_id ) ) );

				// Never leave a product with no category at all.
				if ( ! empty( $keep ) ) {
					wp_set_object_terms( $id, $keep, 'product_cat', false );
					$dropped[] = (int) $default->term_id;
					$touch[]   = (int) $default->term_id;
				}
			}
		}

		if ( empty( $term_ids ) && empty( $dropped ) ) {
			continue;
		}

		$log[ $id ] = array(
			'name'    => $product->get_name(),
			'added'   => $term_ids,
			'slugs'   => $missing,
			'dropped' => $dropped,
		);

		$touch = array_merge( $touch, $term_ids );
	}

	// Term counts drive the category listings, so refresh the ones we changed.
	foreach ( array_unique( $touch ) as $term_id ) {
		wp_update_term_count_now( array( $term_id ), 'product_cat' );
	}

	if ( function_exists( 'wc_delete_product_transients' ) ) {
		wc_delete_product_transients();
	}

	// Repeat until a pass removes nothing. Product categories are hierarchical
	// and a parent is skipped while it still has children, so a single pass only
	// clears the leaves. The first live run left 22 of 48 for exactly this
	// reason. Bounded so a bug can never loop forever.
	$deleted = array();

	for ( $pass = 0; $pass < 6; $pass++ ) {
		$round = opl_pcat_sweep_unused();

		if ( empty( $round ) ) {
			break;
		}

		$deleted = array_merge( $deleted, $round );
	}

	update_option( 'opl_pcat_log', $log, false );
	update_option( 'opl_pcat_deleted', $deleted, false );
	update_option( 'opl_pcat_done_v3', gmdate( 'c' ), false );
}

/**
 * Delete product categories that are empty, unprotected and unreferenced.
 *
 * The site carries 66 product categories, of which 64 were empty - sediment
 * from an earlier supplement catalogue (gut-health, sleep-support,
 * daily-foundation) and an abandoned taxonomy plan (concentration-reference-
 * sets, orii-research-collections). There are also straight duplicates:
 * `research-catalog` is a second term also named "Research Compounds", and
 * there are pairs like `cellular-longevity` / `cellular-longevity-2` and
 * `support-products` / `support-products-support-products`.
 *
 * This is the only destructive operation in the file, so it re-verifies
 * everything at run time rather than trusting a list captured earlier:
 *
 *   1. the term is not in opl_pcat_protected()
 *   2. it holds zero products, counted directly rather than from the cached
 *      `count` column, which was already observed to be out of step
 *   3. it has no child terms, so no hierarchy is orphaned
 *   4. it is not the default category
 *
 * Every deleted term is recorded in `opl_pcat_deleted` with its name, slug,
 * id and parent, which is enough to recreate any of them by hand.
 *
 * Runs AFTER the assignment above, so a term this migration just filled can
 * never be swept. Called repeatedly by opl_pcat_run() until a pass deletes
 * nothing, because a parent term is skipped while it still has children.
 */
function opl_pcat_sweep_unused() {
	$protected = opl_pcat_protected();
	$deleted   = array();

	$terms = get_terms(
		array(
			'taxonomy'   => 'product_cat',
			'hide_empty' => false,
		)
	);

	if ( is_wp_error( $terms ) ) {
		return $deleted;
	}

	$default = (int) get_option( 'default_product_cat' );

	foreach ( $terms as $term ) {
		if ( in_array( $term->slug, $protected, true ) ) {
			continue;
		}

		if ( (int) $term->term_id === $default ) {
			continue;
		}

		// Count products directly. `$term->count` proved unreliable on this
		// site, and deleting on the strength of a stale zero would lose a
		// category that actually holds products.
		$in_use = get_posts(
			array(
				'post_type'        => 'product',
				'post_status'      => array( 'publish', 'draft', 'pending', 'private', 'future' ),
				'numberposts'      => 1,
				'fields'           => 'ids',
				'suppress_filters' => false,
				'tax_query'        => array(
					array(
						'taxonomy'         => 'product_cat',
						'field'            => 'term_id',
						'terms'            => (int) $term->term_id,
						'include_children' => true,
					),
				),
			)
		);

		if ( ! empty( $in_use ) ) {
			continue;
		}

		// Never orphan a child term.
		$children = get_term_children( (int) $term->term_id, 'product_cat' );

		if ( ! is_wp_error( $children ) && ! empty( $children ) ) {
			continue;
		}

		$record = array(
			'id'     => (int) $term->term_id,
			'name'   => $term->name,
			'slug'   => $term->slug,
			'parent' => (int) $term->parent,
		);

		$result = wp_delete_term( (int) $term->term_id, 'product_cat' );

		if ( true === $result ) {
			$deleted[] = $record;
		}
	}

	return $deleted;
}
