...
/*
 * Every function is suffixed `4`. An earlier build of this snippet guarded
 * itself with function_exists() on unsuffixed names, so if a stale copy is
 * still loaded anywhere - the other snippets plugin, a duplicate snippet - it
 * defines those names first and silently skips the whole of a newer paste.
 * Unique names make this build immune to that.
 */
if ( ! function_exists( 'opl_facts4_map' ) ) {

/** Bullet copy. Keys match as substrings of the lowercased product name. */
function opl_facts4_map() {
	return array(
		// Tirzepatide
		'tirzepatide' => array(
...
function opl_facts4_html( $product_name ) {
	...
	foreach ( opl_facts4_map() as $key => $lines ) {
...
function opl_facts4_count( $reset = false ) {
	static $n = 0;
...
function opl_facts4_card( $matches ) {
	$card = $matches[0];
	...
	$facts = opl_facts4_html( $heading[1] );
...
function opl_facts4_cat_card( $matches ) {
	$card = $matches[0];
	...
	foreach ( opl_facts4_cat_map() as $key => $candidate ) {
 ...
 */
function opl_facts4_apply( $html ) {
...
function opl_facts4_style_tag() {
	return '<style id="opl-facts4-css">' . opl_facts4_css() . '</style>';
}

/*
 * Pass 1 - the_content. Handles the homepage grid and the /research/ cards.
 */
add_filter( 'the_content', 'opl_facts4_filter', PHP_INT_MAX );

function opl_facts4_filter( $content ) {
	if ( is_admin() || is_feed() ) {
		return $content;
	}

	opl_facts4_count( true );
	$out = opl_facts4_apply( $content );
	$GLOBALS['opl_facts_last'] = opl_facts4_count() - 1;

	if ( null === $out ) {
		return $content;
	}

	return opl_facts4_style_tag() . $out
		. '<!-- opl-facts v4 pass1 rewrote=' . $GLOBALS['opl_facts_last'] . ' -->';
}
 ...
 */
add_action( 'template_redirect', 'opl_facts4_buffer_start', PHP_INT_MAX );

function opl_facts4_buffer_start() {
	if ( is_admin() || is_feed() || is_robots() ) {
		return;
	}

	ob_start( 'opl_facts4_buffer' );
}

function opl_facts4_buffer( $html ) {
	if ( ! is_string( $html ) || '' === $html ) {
		return $html;
	}
	...
	opl_facts4_count( true );
	$out = opl_facts4_apply( $html );
	$rewrote = opl_facts4_count() - 1;

	$marker = '<!-- opl-facts v4 pass2 seen=' . (int) $seen . ' rewrote=' . $rewrote . ' -->';
	...
	if ( false === strpos( $out, 'opl-facts4-css' ) ) {
		$patched = preg_replace( '#</head>#i', opl_facts4_style_tag() . '</head>', $out, 1 );
